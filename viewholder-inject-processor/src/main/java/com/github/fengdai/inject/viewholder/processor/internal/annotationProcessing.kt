...
import com.github.fengdai.inject.viewholder.processor.internal.MirrorValue.Array
import com.github.fengdai.inject.viewholder.processor.internal.MirrorValue.Error
import com.github.fengdai.inject.viewholder.processor.internal.MirrorValue.Type
import com.github.fengdai.inject.viewholder.processor.internal.MirrorValue.Unmapped
...
/** Return the first `T` annotation. */
inline fun <reified T : Annotation> AnnotatedConstruct.getAnnotation() =
  getAnnotation(T::class.java.canonicalName)
...
fun AnnotationValue.toMirrorValue(): MirrorValue = accept(
    MirrorValueVisitor, null)

sealed class MirrorValue {
  data class Type(val value: TypeMirror) : MirrorValue(), TypeMirror by value
  data class Array(val value: List<MirrorValue>) : MirrorValue(), List<MirrorValue> by value
...
private object MirrorValueVisitor : SimpleAnnotationValueVisitor6<MirrorValue, Nothing?>() {
  ...
  override fun visitType(mirror: TypeMirror, ignored: Nothing?) = mirror.accept(
      TypeVisitor, null)
